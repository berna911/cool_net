...
class Model_friends extends Model
{
	public function get_data()
	{
		...
		$users = DB::getInstance() -> query('SELECT * FROM us_main WHERE id = '.$_COOKIE['uid']);
		foreach ($users -> results() as $user);

		$user_friends = DB::getInstance() -> query('SELECT rel_friends FROM us_main WHERE id='.$_COOKIE['uid']);
